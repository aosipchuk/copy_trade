...
async def reconcile_demo_targets(
    ...
    async with get_db_session() as db:
        ...
        if subscription is None:
            return 0
        return await reconcile_demo_targets_in_session(
            db,
            subscription,
...
async def reconcile_demo_targets_in_session(
    db: AsyncSession,
    subscription: Subscription,
    *,
    market_keys: list[str] | None = None,
) -> int:
    """Reconcile demo targets inside the caller's transaction."""
    if (
        not subscription.is_demo
        or subscription.engine_version != 2
        or subscription.execution_status not in ("active", "stopping")
        or (subscription.execution_status == "active" and not subscription.is_active)
    ):
        return 0
    snapshot = await MarketRegistry().get_snapshot()
    changed = 0
    ...
    for target in targets:
        ...
        if spec is None or spec.mid is None:
            ...
            continue
        idempotency_key = f"demo:{subscription_id}:{target.id}:{target.target_version}"
